...
<form id="Form1" name="Form1" action="/dataChart/queryFiles.html" method="post" style="margin:0px;">
				...
				<tr>
					...
					<td class="ta_01" >
						...
						<select name="projId" id="projId" style="width:160px">
							...
							<c:forEach items="${allProjIDs}" var="projid">
								<option value="${projid.ddlcode}-${projid.ddlname}"
										<c:if test="(${projid.ddlcode}+'-'+${projid.ddlname})==${param.projId}">
											  selected

										</c:if>
								>${projid.ddlname}</option>
							</c:forEach>
					...
					<td class="ta_01" >
						...
						<select name="belongTo" id="belongTo" style="width:160px">
						    <option value="" selected="selected">全部</option>
							<c:forEach items="${allBelongTos}" var="belongTo">
								<option value="${belongTo.ddlcode}-${belongTo.ddlname}"
										<c:if test="(${belongTo.ddlcode}-${belongTo.ddlname})==${param.belongTo}">
											  selected

										</c:if>
								>${belongTo.ddlname}</option>
							</c:forEach>
						</select>
		     ...
		     <tr>
		        <td class="ta_01" align="center" bgcolor="#f5fafe" height="22">
		          ...
		          <td class="ta_01">
		          	...
		          	<input type="text" name="queryString" size="21" value="${param.queryString}" id="queryString" onkeydown="returnMethod()"/>
		          </td>
	...
	<TR>
		...
		<td class="ta_01" align="right" bgcolor="#ffffff">
			...
			<%--<%--%>
				...
				<%--if(CheckPopeUtil.hasPope("mb",session)){--%>
			<%--%>--%>
			...
			<table cellspacing="0" cellpadding="1" rules="all" bordercolor="gray" border="1" id="DataGrid1" style="BORDER-RIGHT:gray 1px solid; BORDER-TOP:gray 1px solid; BORDER-LEFT:gray 1px solid; WIDTH:100%; WORD-BREAK:break-all; BORDER-BOTTOM:gray 1px solid; BORDER-COLLAPSE:collapse; BACKGROUND-COLOR:#f5fafe; WORD-WRAP:break-word">
				<tr style="FONT-WEIGHT:bold;FONT-SIZE:12pt;HEIGHT:25px;BACKGROUND-COLOR:#afd1f3">
					...
					<td width="6%" align="center" height=22 background="${pageContext.request.contextPath }/images/tablehead.jpg">删除</td>
						...
						<c:forEach items="${elecFileuploads}" var="fileItem" varStatus="vs">


						<tr onmouseover="this.style.backgroundColor = 'white'" onmouseout="this.style.backgroundColor = '#F5FAFE';">
							<td  align="center" width="6%">${vs.count}</td>
							<td align="center"  width="10%">${fileItem.projid}</td>
							<td align="center"  width="10%">${fileItem.belongto}</td>
							<td align="center"  width="48%">
							<a href="#" onclick="openWindow('${fileItem.fileurl}','700','400');">${fileItem.filename}</a></td>
							<td align="center"  width="20%">${fileItem.comment}</td>
